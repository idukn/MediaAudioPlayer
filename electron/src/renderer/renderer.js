...
const applyDirBtn = document.getElementById('applyDirBtn');
...
const ctxSaveSelectedAudioBtn = document.getElementById('ctxSaveSelectedAudio');
const ctxSaveSelectedEachBtn = document.getElementById('ctxSaveSelectedEach');
const ctxSaveSelectedNewFolderBtn = document.getElementById('ctxSaveSelectedNewFolder');
...
function openSearchContextMenu(x, y) {
  ...
  ctxSaveSelectedAudioBtn.style.display = selectedCount <= 1 ? 'block' : 'none';
  ctxSaveSelectedEachBtn.style.display = selectedCount > 1 ? 'block' : 'none';
  ctxSaveSelectedNewFolderBtn.style.display = selectedCount > 1 ? 'block' : 'none';
...
async function saveSelectedSearchAudio({ toNewFolderForMultiple = false } = {}) {
  ...
  let targetDir = saveDirEl.value.trim();
  if (!targetDir) {
    safeAlert('保存先パスが設定されていません。');
    return;
  }
  ...
  if (toNewFolderForMultiple && items.length > 1) {
    ...
    const created = await window.api.createFolder({ parentDir: targetDir, name: folderName });
...
async function moveSelectedToNewFolder() {
  ...
  const parentDir = currentAudioDir || saveDirEl.value.trim();
  if (!parentDir) {
...
async function refreshLocal() {
  try {
    const baseDir = saveDirEl.value.trim();
    const listDir = currentAudioDir || baseDir;
...
async function boot() {
  ...
  const dir = await window.api.getSaveDir();
  audioServerPort = await window.api.getAudioServerPort();
  ...
  if (savedState) {
    ...
    currentAudioDir = savedState.audioDir || '';
  ...
  // Keep persisted UI state consistent after boot restoration.
  persistUiState();
...
chooseDirBtn.addEventListener('click', async () => {
  try {
    ...
    if (chosen) {
      saveDirEl.value = chosen;
      await refreshLocal();
      setStatus('Save path updated');
    }
  } catch (error) {
    safeAlert(`保存先フォルダを設定できません。\n${error.message}`);
  }
});

applyDirBtn.addEventListener('click', async () => {
  const saveDir = saveDirEl.value.trim();
  if (!saveDir) {
    safeAlert('保存先パスを入力してください。');
    return;
  }

  try {
    await window.api.setSaveDir(saveDir);
    baseAudioDir = saveDir;
    currentAudioDir = '';
    selectedLocalIndex = -1;
    await refreshLocal();
    updateBackButtonVisibility();
...
ctxSaveSelectedAudioBtn.addEventListener('click', async () => {
  closeSearchContextMenu();
  try {
    await saveSelectedSearchAudio({ toNewFolderForMultiple: false });
  } catch (error) {
    safeAlert(`保存に失敗しました。\n${error.message}`);
  }
});

ctxSaveSelectedEachBtn.addEventListener('click', async () => {
  closeSearchContextMenu();
  try {
    await saveSelectedSearchAudio({ toNewFolderForMultiple: false });
  } catch (error) {
    safeAlert(`複数保存に失敗しました。\n${error.message}`);
  }
});

ctxSaveSelectedNewFolderBtn.addEventListener('click', async () => {
  closeSearchContextMenu();
  try {
    await saveSelectedSearchAudio({ toNewFolderForMultiple: true });
  } catch (error) {
    safeAlert(`新規フォルダ保存に失敗しました。\n${error.message}`);
  }
});
